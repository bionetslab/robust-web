...
def celery_running_task(custom_id, input_dict):
    ...
    record = Robust(custom_id, input_dict["path_to_graph"], input_dict["seeds"], input_dict["namespace"], input_dict["alpha"], input_dict["beta"], input_dict["n"], input_dict["tau"], input_dict["study_bias_score"], input_dict["study_bias_score_data"], input_dict["gamma"], input_dict["in_built_network"], input_dict["provided_network"], input_dict["is_graphml"], nodeData_str, edgeDataSrc_str, edgeDataDest_str, is_seed_str)
...
class Robust(db.Model):
    ...
    def __init__(self, custom_id, path_to_graph, seeds, namespace, alpha, beta, n, tau, study_bias_score, study_bias_score_data, gamma, in_built_network, provided_network, is_graphml, nodeData_str, edgeDataSrc_str, edgeDataDest_str, is_seed_str):
...
def results():
    if flask.request.method == 'POST':
        ...
        provided_network, ppi_network_contents_df, is_graphml=_get_network_contents(is_graphml, in_built_network, ppi_network_contents_df, uploaded_network, NETWORK)
        custom_studybiasdata_input_df=pd.DataFrame()
        study_bias_score_data, custom_studybiasdata_input_df=_get_study_bias_data_contents(custom_studybiasdata_input_df, study_bias_score)
        ...
        if not error_statement=='None':
            return render_template('run_robust.html', error=error_statement)
        custom_studybiasdata_input_df, error_statement=_custom_study_bias_data_error(study_bias_score, custom_studybiasdata_input_df)
        if not error_statement=='None':
            return render_template('run_robust.html', error=error_statement)
...
def _make_input_dict(path_to_graph, seeds, namespace, alpha, beta, n, tau, study_bias_score, study_bias_score_data, gamma, in_built_network, provided_network, is_graphml):
    input_dict={
...
if __name__=='__main__':
    scheduler.add_job(id = 'Updation of study bias scores', func=update_study_bias_scores_scheduled_task, trigger="interval", seconds=30000)
